...
This guide provides a complete implementation of the end-to-end payment and payout system for Tikèm, from customer ticket purchase to organizer payouts. The system is built on the existing Firebase/Firestore infrastructure and extends it with new collections and enhanced workflows.
...
interface EventEarnings {
  ...
  // Fee breakdown
  platformFee: number            // Tikèm's cut (e.g., 10%)
