...
export function DataTable<T>({
  ...
  const SortIcon = ({ col }: { col: Column<T> }) => {
    ...
    if (activeSortKey !== col.key) return <ChevronsUpDown className="h-3.5 w-3.5 text-gray-400" />
  ...
  return (
    <div
      className={`overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-soft ${className}`}
    >
      {toolbar && <div className="border-b border-gray-100 p-3 sm:p-4">{toolbar}</div>}
      ...
      {loading ? (
        ...
        <div className="p-8">
          {empty ?? <p className="text-center text-sm text-gray-500">No results found.</p>}
        </div>
      ) : (
        <>
          {/* Desktop table */}
                ...
                <tr className="border-b border-gray-100">
                  ...
                  {columns.map((col) => (
                    <th
                      ...
                      className={`px-4 py-3 text-[11px] font-semibold uppercase tracking-wider text-gray-500 sm:px-6 ${
                  ...
                  return (
                    ...
                    >
                      ...
                      {columns.map((col) => (
                        <td
                          key={col.key}
                          className={`px-4 py-3 align-middle text-sm text-gray-900 sm:px-6 ${
                            alignClass[col.align ?? 'left']
                          } ${col.cellClassName ?? ''}`}
                        >
                          {col.render
                ...
                >
                  {columns.map((col) => (
                    <div key={col.key} className="flex items-start justify-between gap-3">
                      ...
                      <span className="min-w-0 text-right text-sm text-gray-900">
                        {col.render
                          ? col.render(row)
                          : String((row as Record<string, unknown>)[col.key] ?? '')}
                      </span>
