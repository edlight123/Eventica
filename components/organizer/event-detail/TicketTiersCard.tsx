    ...
    <div className="bg-[#141414] rounded-xl border border-white/10 p-5 shadow-sm">
      ...
      {!hasTiers ? (
        <div className="text-center py-8 border-2 border-dashed border-white/10 rounded-lg">
          <Ticket className="w-12 h-12 text-white/50 mx-auto mb-3" />
          <p className="text-sm text-white/60 mb-3">{t('organizer.no_ticket_tiers_configured')}</p>
          <Link
            href={`/organizer/events/${eventId}/edit#tickets`}
                ...
                <div className="flex items-center justify-between text-sm">
                  <div className="flex items-center gap-3">
                    <span className="text-white/60">
                      <span className="font-semibold text-white">{tier.sold}</span> / {tier.quantity} sold
                    </span>
                  </div>
