  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'event_published':
        return <Share2 className="w-4 h-4 text-brand-300" />
      case 'event_updated':
        return <Edit className="w-4 h-4 text-brand-300" />
      default:
  ...
  return (
    <div className="bg-[#0a0a0a] rounded-xl  p-5 shadow-sm">
      ...
      {activities.length > 10 && (
        <button className="mt-4 text-sm text-brand-300 hover:text-brand-300 font-medium">
