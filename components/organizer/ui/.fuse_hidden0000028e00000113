...
export function OrgDataTable<T>({
  ...
  return (
    <div
      className={`overflow-hidden rounded-2xl border border-white/10 bg-[#141414] ${className}`}
    >
      {toolbar && (
        ...
        <div className="divide-y divide-white/5">
          {Array.from({ length: skeletonRows }).map((_, i) => (
            <div key={i} className="flex animate-pulse items-center gap-4 px-4 py-4 sm:px-6">
              <div className="h-9 w-9 rounded-lg bg-[#242424]" />
              <div className="flex-1 space-y-2">
                <div className="h-3.5 w-1/3 rounded bg-[#242424]" />
                <div className="h-3 w-1/4 rounded bg-[#242424]" />
              </div>
              <div className="h-6 w-16 rounded-full bg-[#242424]" />
            </div>
          ))}
        </div>
      ) : rows.length === 0 ? (
        ...
        <div className="flex items-center justify-between border-t border-white/10 px-4 py-3 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1 rounded-lg border border-white/10 px-3 py-1.5 text-sm font-medium text-white/70 transition-colors hover:bg-[#1c1c1c] disabled:cursor-not-allowed disabled:opacity-40"
            >
