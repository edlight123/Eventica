    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
          <h3 className="font-display text-xl text-gray-900">Request payout</h3>
          <button onClick={close} disabled={status === 'loading'} aria-label="Close" className="text-gray-400 transition-colors hover:text-gray-600 disabled:opacity-50">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        {status === 'success' ? (
          <div className="px-5 py-8 text-center">
            <div className="mx-auto mb-3 grid h-12 w-12 place-items-center rounded-full bg-green-100 text-green-700">
              <CheckCircle2 className="h-6 w-6" />
            </div>
            <h4 className="font-display text-lg text-gray-900">Payout requested</h4>
          ...
          <div className="space-y-4 px-5 py-5">
            <div className="rounded-xl border border-gray-100 bg-gray-50 p-4">
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-gray-500">
                <Wallet className="h-4 w-4 text-brand-700" /> Amount
              </div>
              <p className="mt-1 text-2xl font-bold text-gray-900">{availableLabel}</p>
            ...
            <div className="flex gap-3 pt-1">
              <button onClick={close} disabled={status === 'loading'} className="flex-1 rounded-lg border border-gray-300 px-4 py-2.5 text-sm font-semibold text-gray-700 transition-colors hover:bg-gray-50 disabled:opacity-50">
                Cancel
              </button>
