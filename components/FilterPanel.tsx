...
export default function FilterPanel({
  ...
  return (
    <>
      ...
      `}>
        {/* Header */}
        <div className="sticky top-0 bg-[#0a0a0a] border-b border-white/10 px-4 py-4 flex items-center justify-between z-10">
          <div className="flex items-center gap-3">
            <SlidersHorizontal className="w-5 h-5 text-white/70" />
            <div>
              <h2 className="text-lg font-bold text-white">{t('filters.filters')}</h2>
              {activeCount > 0 && (
                <p className="text-xs text-white/50">{activeCount} {t('filters.active')} {t('filter')}{activeCount !== 1 ? 's' : ''}</p>
              )}
            </div>
          </div>
          <button
        ...
        <div className="overflow-y-auto h-[calc(100vh-140px)] md:h-[calc(100vh-160px)] px-4 py-6 space-y-6">
            ...
            <div className="space-y-2">
              ...
              {showDatePicker && (
                <input
                  ...
                  className="ml-7 mt-2 px-3 py-2 border border-white/10 rounded-lg text-sm focus:ring-2 focus:ring-brand-500 focus:outline-none"
                />
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-white/10 rounded-lg text-sm focus:ring-2 focus:ring-brand-500 focus:outline-none"
            >
            ...
            {showSubdivisions && (
              <select
                ...
                className="w-full mt-2 px-3 py-2 border border-white/10 rounded-lg text-sm focus:ring-2 focus:ring-brand-500 focus:outline-none"
              >
        ...
        {/* Footer Actions */}
        <div className="sticky bottom-0 bg-[#0a0a0a] border-t border-white/10 px-4 py-4 flex gap-3">
          <button
            onClick={onReset}
            disabled={!hasChanges && activeCount === 0}
            className="flex-1 px-4 py-3 border-2 border-white/10 text-white/70 font-semibold rounded-xl hover:bg-[#0a0a0a] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            {t('filters.reset')}
          </button>
          <button
