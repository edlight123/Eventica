    ...
    <div className={`rounded-2xl border-2 border-dashed p-12 text-center hover:border-brand-400 transition-colors duration-300 ${dark ? 'border-white/15' : 'bg-white/[0.03] border-white/10'}`}>
      ...
      <div className="relative inline-block mb-6">
        ...
        <div className={`relative rounded-full p-6 ${dark ? '' : 'bg-gradient-to-br from-gray-50 to-gray-100'}`}>
          <Icon className={`w-16 h-16 ${dark ? 'text-white/40' : 'text-white/40'}`} strokeWidth={1.5} />
        </div>
      </div>

      {/* Title */}
      <h3 className={`text-2xl font-bold mb-3 ${dark ? 'text-white' : 'text-white'}`}>
        {title}
      </h3>

      {/* Description */}
      <p className={`mb-6 max-w-md mx-auto leading-relaxed ${dark ? 'text-white/55' : 'text-white/65'}`}>
        {description}
      </p>
