...
export default function TicketTiersManager({ eventId }: TicketTiersManagerProps) {
  ...
  if (loading && tiers.length === 0) {
    return <p className="text-white/65">Loading tiers...</p>
  }
  ...
  return (
    <div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-[#0a0a0a] border border-white/10 rounded-lg p-4 mb-4">
      ...
      {tiers.length === 0 ? (
        ...
        <div className="space-y-3">
          {tiers.map((tier) => (
            <div key={tier.id} className="bg-[#0a0a0a] border border-white/10 rounded-lg p-4">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 mt-2 text-sm text-white/65">
