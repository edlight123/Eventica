...
export function AdminActivityFeed({ 
  ...
  const getActivityColor = (type: string, severity?: string) => {
    ...
    if (severity === 'high') return 'text-amber-300 bg-amber-500/10'
    
    switch (type) {
      case 'verification':
        return 'text-brand-300 bg-brand-500/10'
      case 'payment':
      ...
      case 'user_action':
        return 'text-brand-300 bg-brand-500/10'
      case 'system':
        return 'text-white/60 bg-[#0a0a0a]'
      default:
        return 'text-brand-300 bg-brand-500/10'
    }
  }
              ...
              return (
                <div key={activity.id}>
                  ...
                  >
                    ...
                    {hasDetails && (
                      <ChevronDown
                        className={`w-3.5 h-3.5 text-white/50 flex-shrink-0 transition-transform ${
                  ...
                  {isExpanded && hasDetails && (
                    <div className="pl-[3.125rem] pr-3 pb-2.5 -mt-0.5 space-y-1.5">
                      {detailText && (
                        <p className="text-xs text-white/60">{detailText}</p>
                      )}
                      {(activity.metadata?.amount != null || activity.metadata?.severity || activity.link) && (
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-white/50">
                          ...
                          {activity.link && (
                            <Link
                              href={activity.link}
                              className="text-brand-300 hover:text-brand-300 inline-flex items-center gap-0.5 font-medium"
                            >
