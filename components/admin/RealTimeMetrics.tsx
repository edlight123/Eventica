      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 sm:gap-3">
        {metrics.map((metric) => {
          ...
          return (
            <div
              key={metric.title}
              className="bg-white rounded-xl border border-gray-200 p-3 sm:p-3.5 hover:border-gray-300 transition-colors"
            >
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center ring-1 ${colorClasses} mb-2`}>
                <Icon className="w-4 h-4" />
              </div>
              <div className="text-lg sm:text-xl font-bold text-gray-900 leading-none tabular-nums">
                {metric.value}
              </div>
