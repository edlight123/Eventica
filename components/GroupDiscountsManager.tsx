...
export default function GroupDiscountsManager({ eventId }: GroupDiscountsManagerProps) {
  ...
  if (loading && discounts.length === 0) {
    return <p className="text-white/65">Loading...</p>
  }

  return (
    <div className="bg-[#0a0a0a] rounded-lg shadow-sm border border-white/10 p-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-lg font-semibold text-white">Group Discounts</h3>
          <p className="text-sm text-white/65">Encourage bulk purchases with quantity-based discounts</p>
        </div>
        {!showForm && (
          <button
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="mb-6 bg-[#0a0a0a] rounded-lg p-4 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-white/10 rounded-lg focus:ring-2 focus:ring-teal-500"
                placeholder="e.g., 4"
              />
              <p className="text-xs text-white/50 mt-1">Min tickets to qualify</p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g., 10"
              />
              <p className="text-xs text-white/50 mt-1">Percentage off</p>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-4 py-2 bg-white/[0.06] text-white/70 rounded-lg hover:bg-gray-300"
            >
              Cancel
            </button>
          </div>
            ...
            .map((discount) => (
              ...
              >
                ...
                <button
                  ...
                  disabled={loading}
                  className="ml-4 text-red-600 hover:text-red-300 disabled:opacity-50"
                >
