...
const PLATFORMS: Array<{
  ...
  { key: 'tiktok', label: 'TikTok', placeholder: 'yourhandle', Icon: Music2, color: 'text-gray-900' },
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
      ...
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-900 mb-1">Social & Bio</h2>
            <p className="text-sm text-gray-600">Let people connect with you off Tikèm</p>
          </div>
        </div>
        {!isEditing ? (
          <button
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={isLoading}
              className="inline-flex items-center gap-1 px-3 py-1.5 text-sm font-medium text-gray-600 hover:bg-gray-100 rounded-lg transition-colors disabled:opacity-50"
            >
              <X className="w-4 h-4" />
              Cancel
            </button>
      ...
      <div className="mb-5">
        ...
        {isEditing ? (
          ...
          <p className="text-sm text-gray-700 whitespace-pre-line">
            {profile.bio?.trim() ? profile.bio : <span className="text-gray-400">No bio yet</span>}
          </p>
        )}
      </div>
      ...
      <div className="space-y-3">
        {PLATFORMS.map(({ key, label, placeholder, Icon, color }) => {
          ...
          return (
            <div key={key} className="flex items-center gap-3">
              ...
              {isEditing ? (
                <div className="flex-1 flex items-center rounded-lg border border-gray-300 focus-within:ring-2 focus-within:ring-teal-500 focus-within:border-transparent overflow-hidden">
        ...
        {!isEditing && !hasAnyLink && (
          <p className="text-sm text-gray-400">No social accounts added yet.</p>
        )}
      </div>
