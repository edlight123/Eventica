...
This guide will help you set up Stripe, MonCash (NatCash), and CashApp for accepting payments on Tikèm.
...
- Tikèm uses the same `MONCASH_CLIENT_ID` / `MONCASH_SECRET_KEY` OAuth token to call the prefunded endpoints.
...
Create or update `/workspaces/Tikèm/.env.local` with all keys:
