  ...
  return (
    <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg overflow-hidden">
      ...
      <div className="p-4 sm:p-6">
        {/* Sender Info */}
        <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-brand-50 border border-brand-100 rounded-lg">
          <p className="text-[13px] sm:text-sm text-brand-800 mb-1">
            <strong>{sender?.name || sender?.full_name || 'Someone'}</strong> wants to transfer a ticket to you
          </p>
          <p className="text-[11px] sm:text-xs text-brand-600">
        ...
        {event && (
          <div className="mb-4 sm:mb-6">
            <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-3 sm:mb-4">{String(event.title || '').replace(/^\[[^\]]*\]\s*/, '')}</h2>
            
            <div className="space-y-2.5 sm:space-y-3 text-gray-700">
              <div className="flex items-start">
                <svg className="w-4 h-4 sm:w-5 sm:h-5 mr-2 sm:mr-3 mt-0.5 flex-shrink-0 text-brand-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {/* Info */}
        <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-gray-50 border border-gray-100 rounded-lg">
          <h3 className="text-[13px] sm:text-base font-semibold text-gray-900 mb-1.5 sm:mb-2">What happens when you accept?</h3>
          <ul className="text-[11px] sm:text-sm text-gray-700 space-y-0.5 sm:space-y-1">
