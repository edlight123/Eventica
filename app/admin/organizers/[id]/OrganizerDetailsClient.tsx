...
export default function OrganizerDetailsClient({ organizerDetails }: OrganizerDetailsProps) {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-6">
      {/* Breadcrumb */}
      <div className="mb-4">
        <Link href="/admin/organizers" className="text-sm font-medium text-console-mut hover:text-console-text">
          ← Back to Organizers
        </Link>
      </div>
        ...
        <div className="flex flex-col gap-2 sm:min-w-[200px]">
          {isVerified ? (
            <button
              ...
              disabled={isUpdating}
              className="rounded-lg bg-console-panel px-4 py-2.5 text-sm font-semibold text-console-mut hover:bg-console-raise disabled:opacity-50"
            >
              Remove Verification
            </button>
          ) : (
            <button
              ...
              disabled={isUpdating}
              className="rounded-lg bg-console-text px-4 py-2.5 text-sm font-bold text-console-ground hover:opacity-90 disabled:opacity-50"
            >
              ✓ Verify Organizer
            </button>
          )}
      ...
      <div className="mb-6 grid grid-cols-3 divide-x divide-console-raise overflow-hidden rounded-lg bg-console-panel">
        <div className="p-4">
          <div className="label-mono flex items-center gap-1.5 text-[11px] uppercase tracking-wide text-console-mut">
            <svg className="h-3.5 w-3.5 text-console-faint" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            Total Events
          </div>
          <div className="mt-1 font-mono text-2xl font-bold tabular-nums text-console-text">{stats.totalEvents}</div>
        </div>

        <div className="p-4">
          <div className="label-mono flex items-center gap-1.5 text-[11px] uppercase tracking-wide text-console-mut">
            <svg className="h-3.5 w-3.5 text-console-faint" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clipRule="evenodd" />
            </svg>
            Published Events
          </div>
          <div className="mt-1 font-mono text-2xl font-bold tabular-nums text-console-text">{stats.publishedEvents}</div>
        </div>
        ...
        <div className="p-4">
          ...
          <div className="mt-1 font-mono text-2xl font-bold tabular-nums text-console-text">{stats.ticketsSold}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-lg bg-console-panel p-4 sm:p-5">
          <h2 className="mb-4 flex items-center gap-2 text-base font-semibold text-console-text">
          ...
          <dl className="space-y-3">
            ...
            <div>
              ...
              <dd className={`label-mono uppercase text-sm font-semibold ${isBanned ? 'text-console-red' : 'text-console-green'}`}>
                {isBanned ? 'Banned' : 'Active'}
              </dd>
            </div>
            <div>
              <dt className="text-xs text-console-mut">Can Create Events</dt>
              <dd className={`label-mono uppercase text-sm font-semibold ${canPost ? 'text-console-green' : 'text-console-red'}`}>
                {canPost ? 'Yes' : 'No'}
              </dd>
            </div>
            <div>
        ...
        <div className="rounded-lg bg-console-panel p-4 sm:p-5">
          ...
          {(payoutConfig || (payoutDestinations && payoutDestinations.length > 0)) ? (
              ...
              {payoutDestinations && payoutDestinations.length > 0 && (
                <div className="space-y-3">
                  ...
                  {payoutDestinations.map((dest, index) => (
                    <div key={dest.id || index} className="rounded-lg bg-console-panel p-4">
                      <div className="mb-2 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs font-semibold">
                        ...
                        {dest.status && (
                          <span className={`label-mono uppercase ${
                            dest.status === 'active' || dest.status === 'verified' ? 'text-console-green' :
                            dest.status === 'pending' ? 'text-console-amber' :
                            'text-console-red'
                          }`}>
                      ...
                      <dl className="space-y-1 text-sm">
                        {(dest.accountName || dest.bankDetails?.accountName) && (
                          <div className="flex justify-between gap-3">
                            ...
                            <dd className="text-console-text font-medium">{safeString(dest.accountName || dest.bankDetails?.accountName)}</dd>
                          </div>
                        )}
                        {(dest.bankName || dest.bankDetails?.bankName) && (
                          <div className="flex justify-between gap-3">
                            <dt className="text-console-mut">Bank</dt>
                            <dd className="text-console-text capitalize">{safeString(dest.bankName || dest.bankDetails?.bankName)}</dd>
                          </div>
                        )}
                        ...
                        {(dest.routingNumber || dest.bankDetails?.routingNumber) && (
                          <div className="flex justify-between gap-3">
                            ...
                            <dd className="text-console-text font-mono">{safeString(dest.routingNumber || dest.bankDetails?.routingNumber)}</dd>
                          </div>
                        )}
                        {(dest.accountLocation || dest.bankDetails?.accountLocation) && (
                          <div className="flex justify-between gap-3">
                            <dt className="text-console-mut">Location</dt>
                            <dd className="text-console-text capitalize">{safeString(dest.accountLocation || dest.bankDetails?.accountLocation)}</dd>
                          </div>
                        )}
                        {(dest.provider || dest.mobileMoneyDetails?.provider) && (
                          <div className="flex justify-between gap-3">
                            <dt className="text-console-mut">Provider</dt>
                            <dd className="text-console-text">{safeString(dest.provider || dest.mobileMoneyDetails?.provider)}</dd>
                          </div>
                        )}
                        {(dest.phoneNumber || dest.mobileMoneyDetails?.phoneNumber) && (
                          <div className="flex justify-between gap-3">
                            <dt className="text-console-mut">Phone</dt>
                            <dd className="text-console-text font-mono">{safeString(dest.phoneNumber || dest.mobileMoneyDetails?.phoneNumber)}</dd>
                          </div>
                        )}
                      </dl>
                    </div>
              ...
              {payoutConfig?.bankDetails && !payoutDestinations?.length && (
                <div className="rounded-lg bg-console-panel p-4">
                  ...
                  <dl className="space-y-2 text-sm">
                    <div className="flex justify-between gap-3">
                      <dt className="text-console-mut">Bank</dt>
                      <dd className="text-console-text font-medium">{safeString(payoutConfig.bankDetails?.bankName, 'N/A')}</dd>
                    </div>
                    <div className="flex justify-between gap-3">
                    ...
                    {payoutConfig.bankDetails?.routingNumber && (
                      <div className="flex justify-between gap-3">
                        ...
                        <dd className="text-console-text font-mono">{safeString(payoutConfig.bankDetails.routingNumber)}</dd>
                      </div>
                    )}
                    <div className="flex justify-between gap-3">
                      <dt className="text-console-mut">Location</dt>
                      <dd className="text-console-text">{safeString(payoutConfig.accountLocation || payoutConfig.bankDetails?.accountLocation, 'N/A')}</dd>
                    </div>
                  </dl>
                </div>
              )}
                      ...
                      return (
                        <span
                          ...
                          className={`label-mono uppercase ${
                            status === 'verified' ? 'text-console-green' :
                            status === 'failed' ? 'text-console-red' :
                            'text-console-amber'
                          }`}
                        >
                          {type}: {status}
              ...
              <div className="rounded-lg bg-console-panel p-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className={`label-mono uppercase text-xs font-semibold ${
                    payoutConfig.status === 'active' ? 'text-console-green' :
                    payoutConfig.status === 'pending_verification' ? 'text-console-amber' :
                    payoutConfig.status === 'on_hold' ? 'text-console-red' :
                    'text-console-mut'
                  }`}>
                ...
                {payoutConfig.method && (
                  <p className="mt-1 text-xs text-console-mut">Method: {safeString(payoutConfig.method).replace(/_/g, ' ')}</p>
                )}
                {payoutConfig.payoutProvider && (
                  <p className="text-xs text-console-mut">Provider: {safeString(payoutConfig.payoutProvider).replace(/_/g, ' ')}</p>
                )}
              </div>
              ...
              {payoutConfig.mobileMoneyDetails && (
                <div className="rounded-lg bg-console-panel p-4">
                  ...
                  <dl className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between gap-3">
                      <dt className="text-console-mut">Phone</dt>
                      <dd className="text-console-text font-mono">{safeString(payoutConfig.mobileMoneyDetails.phoneNumber)}</dd>
                    </div>
                    <div className="flex justify-between gap-3">
                      <dt className="text-console-mut">Account Name</dt>
            ...
            <div className="py-8 text-center">
              <svg className="mx-auto mb-2 h-10 w-10 text-console-faint" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <p className="text-sm text-console-mut">No payout configuration</p>
            </div>
          )}
        </div>
            ...
            <dl className="space-y-3">
              <div>
                ...
                <dd className={`label-mono uppercase mt-1 text-sm font-semibold ${
                  safeString(verificationRequest.status) === 'approved' ? 'text-console-green' :
                  safeString(verificationRequest.status) === 'rejected' ? 'text-console-red' :
                  'text-console-amber'
                }`}>
              ...
              {verificationRequest.rejection_reason && typeof verificationRequest.rejection_reason === 'string' && (
                <div>
                  ...
                  <dd className="text-sm text-console-red">
          ...
          <div className="rounded-lg bg-console-panel p-4 sm:p-5">
            ...
            <div className="space-y-3">
              {verificationDocs.map((doc) => {
                ...
                return (
                  <div key={doc.id} className="rounded-lg bg-console-panel p-4">
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3">
                        {docType === 'identity' && (
                          <svg className="h-5 w-5 text-console-mut" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2" />
                          </svg>
                        )}
                        {docType === 'bank' && (
                          <svg className="h-5 w-5 text-console-green" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <div className="flex items-center gap-3">
                        <span className={`label-mono uppercase text-xs font-semibold ${
                          status === 'verified' || status === 'approved' ? 'text-console-green' :
                          status === 'rejected' || status === 'failed' ? 'text-console-red' :
                          'text-console-amber'
                        }`}>
                          {status}
                        </span>
        ...
        {organizer && (
          <div className="rounded-lg bg-console-panel p-4 sm:p-5 lg:col-span-2">
            <h2 className="mb-4 text-base font-semibold text-console-text">Organization Profile</h2>
              ...
              {organizer.website && (
                <div>
                  ...
                  <dd className="mt-1 text-sm text-console-text">
                    <a href={organizer.website} target="_blank" rel="noopener noreferrer" className="text-console-mut hover:underline">
                      {organizer.website}
                    </a>
                  </dd>
                </div>
