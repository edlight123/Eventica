    ...
    <>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 sm:py-6 pb-mobile-nav">
        <div className="mb-4 sm:mb-5">
          ...
          />
          {unreviewedCount > 0 && (
            <div className="mt-3 sm:mt-4 bg-red-500/10 border border-red-500/30 rounded-lg p-3 sm:p-4">
              <p className="text-[13px] sm:text-base text-red-300 font-medium">
          ...
          {rebuildStatus && (
            <div className="mt-4 text-sm text-white/70">
              ...
              {rebuildStatus.step !== 'done' && (
                <div className="text-white/50 mt-1">Current step: {rebuildStatus.step}</div>
              )}
              {rebuildStatus.step === 'done' && (
                <div className="text-emerald-300 font-semibold mt-1">Rebuild complete</div>
              )}
              {rebuildStatus.error && (
                <div className="text-red-300 font-semibold mt-1">{rebuildStatus.error}</div>
              )}
            </div>
          )}
        </div>
        ...
        <div className="bg-[#141414] rounded-xl shadow-sm border border-white/10 p-4 sm:p-6 mb-4 sm:mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4">
            <div>
              <label className="block text-[11px] sm:text-sm font-medium text-white/70 mb-1.5 sm:mb-2">
                Review Status
              </label>
              <select
                value={filter.reviewed}
                onChange={(e) => setFilter({ ...filter, reviewed: e.target.value })}
                className="w-full px-3 sm:px-4 py-2 border border-white/15 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-transparent text-[15px] sm:text-base min-h-[44px]"
              >
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      {activity.reviewed && (
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-500/15 text-emerald-300">
                          ✓ Reviewed
                        </span>
                      )}
                    </div>
