    ...
    try {
      ...
      if (isDemoMode()) {
        ...
        setResult({
          ...
          ticket: { ...demoTicket, users: { full_name: 'Demo Attendee', email: 'demo-attendee@tikem.co' } },
          event: demoEvent
        })
        setLoading(false)
        return
      ...
      <form onSubmit={handleValidate} className="space-y-6">
        <div>
          ...
          <textarea
            ...
            className="w-full px-4 py-3 border border-white/15 rounded-lg focus:ring-2 focus:ring-brand-600 focus:border-transparent font-mono text-sm"
            required
          />
            ...
            <div className="mt-4 pt-4 border-t border-white/15">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-semibold text-white">{result.event.title}</p>
                </div>
                <div>
                  <p className="text-white/60">Attendee:</p>
                  <p className="font-semibold text-white">
                    {result.ticket.users?.full_name || 'N/A'}
                  </p>
                </div>
                <div>
                  <p className="text-white/60">Ticket ID:</p>
                  <p className="font-mono text-xs text-white">{result.ticket.id}</p>
                </div>
                <div>
