...
function Avatar({ user, size = 44 }: { user: PublicUserSummary; size?: number }) {
  ...
  return (
    <div
      className="rounded-full overflow-hidden bg-gradient-to-br from-teal-400 to-accent-400 flex items-center justify-center text-white font-semibold flex-shrink-0"
  ...
  return (
    <span
      className={`inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 rounded-full text-xs font-bold ${
        highlight ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-700'
      }`}
    >
      {children}
    </span>
...
function FriendsTab({ overview, onChange }: { overview: Overview; onChange: () => void }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 px-4 divide-y divide-gray-100">
      ...
      <div>
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-teal-500 focus:border-transparent"
          />
          {searching && (
        ...
        {results.length > 0 && (
          <div className="mt-2 bg-white rounded-2xl shadow-sm border border-gray-200 px-4 divide-y divide-gray-100">
            {results.map((r) => (
        ...
        {query.trim().length >= 2 && !searching && results.length === 0 && (
          <p className="text-sm text-gray-500 mt-3 text-center">No people found for “{query}”.</p>
        )}
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h2 className="font-bold text-gray-900">Find friends from contacts</h2>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-xl border-2 border-gray-200 text-gray-700 hover:bg-gray-50 transition-colors"
              >
