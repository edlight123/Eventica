 ...
 * what each market is allowed to do RIGHT NOW.
 *
 * This is an ADDITIVE support layer that sits alongside the existing location /
 * currency config (lib/filters/config.ts: COUNTRIES, CURRENCY_BY_COUNTRY). It
 * does NOT replace them — a later phase reconciles those. The one job of this
 * module is to answer, for a given country:
 ...
 *
 * Contract (keep in lockstep with mobile/lib/countrySupport.ts):
 ...
 *
 * This module imports nothing framework-specific so it is safe on the server
 * (Admin SDK / route handlers) and the browser alike.
 */
